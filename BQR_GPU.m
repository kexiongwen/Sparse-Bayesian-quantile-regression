function[beta_sample]=BQR_GPU(Y,X,Q)
...
c1=(0.5*Q*(1-Q)).^0.5;
c2=(1-2*Q)/(Q*(1-Q));
beta_sample=zeros(S(2),M+burn_in,"gpuArray");
beta_tilde=ones(S(2),1,"gpuArray");
tau_sample=ones(S(2),1,"gpuArray");
omega_sample=ones(S(1),1,"gpuArray");
a_sample=1;
lam_sample=1;
...
for i=2:(M+burn_in)
    ...
    beta_tilde=cgs(GXTD*GXTD'+gpuArray.speye(S(2)),GXTD*DY+GXTD*randn(S(1),1,"gpuArray")+randn(S(2),1,"gpuArray"),1e-3,[],[],[],beta_tilde);
...
beta_sample=gather(beta_sample(:,burn_in+1:end));
